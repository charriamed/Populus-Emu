...
namespace Stump.Server.WorldServer.Game.Maps.Spawns
{
    public class DungeonSpawningPool : SpawningPoolBase
    {
        ...
        private void OnWinnersDetermined(IFight fight, FightTeam winners, FightTeam losers, bool draw)
        {
            ...
            {
                /*if (!pos.Map.IsDungeon())
                {
                    fighter.Character.SendServerMessage("Tu as gagné un Donjondor pour te récompenser de ta bravoure !");
                    fighter.Character.Inventory.AddItem(ItemManager.Instance.CreatePlayerItem(fighter.Character, 13026, 1)); // Donjondor
                }*/
