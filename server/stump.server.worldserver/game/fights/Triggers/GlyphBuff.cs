...
{
    public class GlyphBuff : Glyph
    {
        private List<FightActor> m_actorsInside = new List<FightActor>();

        public GlyphBuff(short id, FightActor caster, Spell castedSpell, EffectDice originEffect, Spell glyphSpell, Cell centerCell, SpellShapeEnum shape, byte minSize, byte size, Color color)
        ...
        private void Enter(FightActor actor)
        {
            ...
            foreach (var effectHandler in handler.GetEffectHandlers())
            {
                ...
                if (effectHandler.IsValidTarget(actor))
