        ...
        public override bool Execute(Npc npc, Character character)
        {
            ...
            if (!(compareTime <= DateTime.Now))
            {
                switch (character.Account.Lang)
                {
                    ...
                    $"Vous ne pouvez pas relancer de combat face à ce dopeul, vouz devez attendre <b>{compareTime.Subtract(DateTime.Now).Hours} heures, {compareTime.Subtract(DateTime.Now).Minutes} minutes</b>",
                    Color.PaleVioletRed);
                        break;
                }
            ...
            switch (character.Account.Lang)
            {
                default:
                    character.SendServerMessage(
                $"Vous ne pouvez pas relancer de combat face au dopeul Dark Vlad, vouz devez attendre <b>{compareTime.Subtract(DateTime.Now).Hours} heures, {compareTime.Subtract(DateTime.Now).Minutes} minutes</b>",
                Color.PaleVioletRed);
                    break;
            }
            character.LeaveDialog();
